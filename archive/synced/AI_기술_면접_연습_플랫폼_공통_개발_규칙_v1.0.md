...
> 현재 구현 기준 원본은 `docs/backend-conventions.md` 입니다.
...
- 기존 미결정 항목 중 구현 기준으로 닫힌 값을 빠르게 확인하는 인덱스다.
- 실제 구현 전에는 반드시 관련 canonical 문서와 값이 같은지 확인한다.

3. `openapi.yaml`
- HTTP 계약, 요청/응답 구조, 인증 방식, 상태코드의 원본이다.

4. ERD / DB 설계 문서 / migration 파일
...
- `archive/originals/`와 `status: deprecated` 문서는 구현의 직접 원본으로 사용하지 않는다.
