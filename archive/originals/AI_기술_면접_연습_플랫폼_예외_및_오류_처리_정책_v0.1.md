...
- 아직 수치나 운영 정책이 확정되지 않은 부분은 `미결정 항목`으로 분리한다.
...
- 필수 입력이 누락된 상태에서 `completed`로 변경하면 안 된다.
...
정책
- `completed` 상태와 결과 저장은 논리적으로 일관되게 처리한다.
...
- private repository 미지원 범위라면 정책 오류 코드로 분리한다.
...
- 재시도 횟수, backoff 정책은 운영 설정으로 분리하는 것이 바람직하다.
...
- `GITHUB_PRIVATE_REPOSITORY_NOT_SUPPORTED`
...
| private repository 미지원 | 422 | GITHUB_PRIVATE_REPOSITORY_NOT_SUPPORTED | 현재는 private repository를 지원하지 않습니다. | false |
...
- private repository 미지원 상태를 403/422 중 어디로 고정할지
- 문서 추출 실패를 502/503 중 어디에 고정할지
