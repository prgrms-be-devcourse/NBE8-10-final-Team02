...
import { connectGithub, refreshGithubConnection } from '@/api/github';
import { getMe } from '@/api/auth';
import type { GithubConnection } from '@/types/github';
import type { Provider } from '@/types/auth';

// GitHub login 형식 검증 (백엔드와 동일한 규칙으로 프론트에서 선검증)
// 영문자·숫자·하이픈만 허용, 1~39자, 하이픈으로 시작/끝 불가
...
export default function GithubConnectPage() {
  ...
  // 현재 사용자의 연결된 provider 목록 (로그인 방식 확인용)
  ...
  async function handleUrlSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // rate limit 안내: 비인증 요청은 시간당 60회 제한
  ...
  // ── GitHub OAuth 로그인 사용자: 저장된 token으로 repo 목록 가져오기 ──
  ...
  // ── GitHub OAuth로 로그인한 사용자 ─────────────────────
  // 이미 로그인 시점에 token이 저장되어 있으므로 별도 OAuth 절차 없이 repo를 바로 가져올 수 있다.
      ...
      <main className="mx-auto max-w-lg px-4 py-12">
        ...
        <div className="rounded border border-blue-100 bg-blue-50 px-4 py-4 mb-6">
          ...
          <p className="text-xs text-blue-700">
            GitHub OAuth 토큰이 이미 저장되어 있습니다.
            아래 버튼을 눌러 repository 목록을 가져오세요.
          </p>
        </div>
        ...
        <p className="mt-4 text-xs text-zinc-400">
          ...
          <br />
          • 이미 연결한 적이 있다면 바로 repository 선택으로 이동하세요.
        </p>

        <button
          onClick={handleGoToRepositories}
          className="mt-3 text-sm underline text-zinc-500"
        >
          이미 연결했다면 → repository 선택으로 이동
        </button>
      </main>
    );
  }

  // ── Google / Kakao로 로그인한 사용자 ──────────────────
  // GitHub 계정이 별도로 없으므로 URL 입력이나 GitHub OAuth 연동이 필요하다.
  return (
    <main className="mx-auto max-w-lg px-4 py-12">
      <h1 className="mb-2 text-2xl font-semibold">GitHub 연결</h1>
      <p className="mb-8 text-sm text-zinc-500">
      ...
      {mode === 'url' && (
        <form onSubmit={handleUrlSubmit} className="flex flex-col gap-4">
          <div>
            ...
            />
            {loginError && (
              <p className="mt-1 text-xs text-red-600">{loginError}</p>
            )}
          </div>
      ...
      {mode === 'oauth' && (
        <div className="flex flex-col gap-4">
          ...
          <p className="text-xs text-zinc-400">
            • GitHub 계정으로 로그인한 경우 아래 버튼으로 추가 동의를 진행하세요.
            <br />
            • Google 또는 Kakao로 로그인한 경우에도 별도로 GitHub를 연동할 수 있습니다.
          </p>

          {/* TODO: 백엔드에 /github/connections/oauth/callback 엔드포인트 구현 후 활성화 */}
