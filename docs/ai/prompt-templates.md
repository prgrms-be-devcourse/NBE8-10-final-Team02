...
```json
{
  ...
  "writingConstraints": {
    ...
    "lengthPolicy": {
      "short": {"targetChars": 500, "hardMaxChars": 700},
      "medium": {"targetChars": 900, "hardMaxChars": 1200},
      "long": {"targetChars": 1400, "hardMaxChars": 1800}
    }
  },
  "existingEditedAnswers": []
}
...
- `USE_CANDIDATE`면 `candidateQuestionTypes` 상위 우선순위부터 로컬 템플릿 매핑을 확인해 deterministic follow-up 1개를 즉시 검토한다.
- 이 `USE_CANDIDATE` 경로는 `ai.interview.followup.generate.v1`를 호출하지 않는다.
...
```json
{
  ...
  "tagMaster": [
    {"tagName": "질문 의도 미충족", "tagCategory": "content"},
    {"tagName": "답변 미완", "tagCategory": "content"},
    {"tagName": "구체성 부족", "tagCategory": "content"},
    {"tagName": "문제 해결 설명 부족", "tagCategory": "content"},
    {"tagName": "답변 구조 미흡", "tagCategory": "structure"},
    {"tagName": "근거 부족", "tagCategory": "evidence"},
    {"tagName": "기술 정확도 부족", "tagCategory": "technical"},
    {"tagName": "기술 깊이 부족", "tagCategory": "technical"},
    {"tagName": "선택 근거 부족", "tagCategory": "technical"},
    {"tagName": "표현 명확성 부족", "tagCategory": "communication"}
  ],
  "scoringRubric": {
    "relevance": 25,
    "structure": 20,
...
규칙:
- isSkipped=true면 score는 0으로 부여한다.
...
- MVP에서는 fallback model 자동 전환을 기본 비활성화한다.
- 운영자가 별도 설정으로 활성화하지 않는 한 동일 모델 재시도만 허용한다.
- fallback을 켠 경우에도 사용자에게는 정상 성공으로 위장하지 않고 내부 로그에 모델 전환 사실을 남겨야 한다.
